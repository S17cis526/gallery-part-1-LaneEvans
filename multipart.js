...
const DOUBL_CRLF = Buffer.from([0x0D,0x0A,0x0D,0x0A]); // Hex code for a double CRLF which indidicates the end of head and start of body


/**
 * @function multipart
 * Takes a request and response object, parses the body of the multipart requests
 * and attaches its contents to the request object. If an error occurs, we log it
 * and send a 500 status code. Otherwise, we invoke next with the request and response.
 */
 ...
 * Takes a buffer and a boundary and returns a associate array of key/value pairs;
 * if content is a file, value will be an object with properties filename, contentType, and
 * data
 */
 ...
 * Parses a content section and returns the key/value pairs as a two-element array
 */
